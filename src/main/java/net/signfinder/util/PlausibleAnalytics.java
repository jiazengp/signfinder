...
public final class PlausibleAnalytics
{
	...
	private static final String MOD_ID = "signfinder";
	 ...
	 * for sending events.
	 */
	public PlausibleAnalytics(ConfigHolder<SignFinderConfig> configHolder,
		KeyBinding toggleKey)
	{
		this.configHolder = Objects.requireNonNull(configHolder);
		this.toggleKey = Objects.requireNonNull(toggleKey);
		
		sessionProp("version", getVersion("signfinder"));
		sessionProp("short_version", getShortVersion("signfinder"));
	...
	private void onWorldChange(MinecraftClient client, ClientWorld world)
	{
		...
		props.put("auto_detection",
			"" + configHolder.get().enable_auto_detection);
		props.put("sign_highlighting",
			"" + configHolder.get().enable_sign_highlighting);
		props.put("highlight_style", configHolder.get().highlight_style.name());
		props.put("auto_remove_nearby",
			"" + configHolder.get().auto_remove_nearby);
	...
	private boolean isDebugMode()
	{
		return FabricLoader.getInstance().isDevelopmentEnvironment()
			|| System.getProperty("signfinder.e2eTest") != null;
	}
