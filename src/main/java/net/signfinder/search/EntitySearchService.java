...
public class EntitySearchService implements SearchService
{
	...
	public List<SignSearchResult> findAllSigns(Vec3d playerPos, int radius)
	{
		...
		for(SignBlockEntity sign : signs)
		{
			...
			signData.ifPresent(
				data -> results.add(new SignSearchResult(sign.getPos(),
	...
	private void searchSigns(SearchQuery query, SignFinderConfig config,
		Vec3d playerPos, Map<BlockPos, EntitySearchResult> resultMap)
	{
		...
		for(SignBlockEntity sign : signs)
		{
			...
			if(queryProcessor.matches(data.combinedText(), query, config))
			{
				...
				resultMap.put(sign.getPos(), result);
			}
		}
	}
		...
		Vec3d playerPos, Map<BlockPos, EntitySearchResult> resultMap)
	{
		...
		for(ItemFrameEntity itemFrame : itemFrames)
		{
				...
				&& queryProcessor.matches(itemName, query, config))
			{
				...
				resultMap.put(itemFrame.getBlockPos(), result);
			}
		}
	}
	
	private List<SignBlockEntity> findSignsInRadius(Vec3d center, int radius)
	{
	...
	private List<ItemFrameEntity> findItemFramesInRadius(Vec3d center,
		int radius)
	{
	 ...
	 * data.
	 */
	private void performFullRangeUpdate(int radius, Vec3d playerPos,
		SignFinderConfig config,
		...
		EntitySearchResult localResult, SignFinderConfig config)
	{
		...
		try
		{
				...
				.getEntityType() == EntitySearchResult.EntityType.SIGN)
			{
				...
				String blockName = mc.world.getBlockState(pos).getBlock()
			...
			}else
			{
				...
				// at this position
				List<ItemFrameEntity> nearbyFrames =
					findItemFramesInRadius(Vec3d.ofCenter(pos), 1);
				boolean hasItemFrame = nearbyFrames.stream()
					.anyMatch(frame -> frame.getBlockPos().equals(pos)
